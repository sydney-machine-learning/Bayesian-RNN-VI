...
class VanillaRNN(nn.Module):
    ...
    def forward(self, x):
        ...
        output= nn.init.kaiming_normal_(torch.zeros(tuple([batch_size, self.output_dim, 1])))
        H_prev = nn.init.kaiming_normal_(torch.zeros(tuple([batch_size, self.hidden_dim, 1])))
