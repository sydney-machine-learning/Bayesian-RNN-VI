...
class BayesianLSTM(nn.Module):
    ...
    def forward(self, x, sampling = False):
        ...
        H_prev,C_prev = (nn.init.kaiming_normal_(torch.empty(tuple([batch_size, 1, self.hidden_dim]))), 
                        nn.init.kaiming_normal_(torch.empty(tuple([batch_size, 1, self.hidden_dim]))))
