...
def get_rmse_list(df):
    ...
    rmse_vals=[]
    for i in track_ids:
        predictions = df[['prediction_latitude','prediction_longitude' ]].loc[df.track_id == i].to_numpy()
        targets= df[['target_latitude','target_longitude' ]].loc[df.track_id == i].to_numpy()
...
plt.bar(x_axis-0.2,rnn_rmse,0.1, color = 'orangered', label ='RNN')
